...
class CMCConnector:
    ...
    async def get_token_metadata(self, symbol):
        """
        Busca metadados e mercado do token via CMC de forma ASSÍNCRONA.
        """
        if not self.api_key:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                info_res = await client.get(info_url, params={"symbol": symbol.upper()}, headers=headers, timeout=15.0)
                ...
                quotes_res = await client.get(quotes_url, params={"symbol": symbol.upper()}, headers=headers, timeout=15.0)
