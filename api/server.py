...
def get_api_key(api_key_header: str = Depends(api_key_header)):
    ...
    if not expected_api_key or not secrets.compare_digest(api_key_header, expected_api_key):
        logger.warning(f"Tentativa de acesso não autorizado com API Key: {api_key_header[:5]}...")
        raise HTTPException(
            status_code=401,
...
async def check_transfer(req: CheckRequest, api_key: str = Depends(get_api_key)):
    try:
        ...
        logger.error(f"Erro em check_transfer: {e}", exc_info=True)
